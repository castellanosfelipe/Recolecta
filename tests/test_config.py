...
def test_base_dir_honors_environment(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setenv("HARVESTER_DATA_DIR", f"  {expected}  ")
...
def test_config_defaults_to_loopback(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.delenv("HARVESTER_PORT", raising=False)
    monkeypatch.delenv("HARVESTER_BIND_LAN", raising=False)
    monkeypatch.delenv("HARVESTER_DASH_USER", raising=False)
    monkeypatch.delenv("HARVESTER_DASH_PASS", raising=False)
...
def test_invalid_ports_have_actionable_error(
    ...
    monkeypatch.setenv("HARVESTER_DATA_DIR", str(tmp_path))
    monkeypatch.setenv("HARVESTER_PORT", value)
    with pytest.raises(ValueError, match="HARVESTER_PORT"):
...
def test_dashboard_credentials_must_be_a_pair(
    ...
    monkeypatch.setenv("HARVESTER_DATA_DIR", str(tmp_path))
    monkeypatch.setenv("HARVESTER_DASH_USER", "operator")
    monkeypatch.delenv("HARVESTER_DASH_PASS", raising=False)
